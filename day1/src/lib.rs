...
// fuel = (mass / 3) >> rounded >> substract 2
...
pub fn total_fuel_requirement(modules: Vec<f64>) -> f64 {
    let mut sum: f64 = 0.;
    for m in modules {
        sum += calc(m);
    }
...
mod tests {
    ...
    #[test]
    fn it_calculates_total_fuel() {
        let modules: Vec<f64> = vec![200., 3000.];
        assert_eq!(total_fuel_requirement(modules), 1062.);
    }
}
